		...
		for eq in self.equal_pattern.keys():
			...
			if re.findall(eq, pos):
				...
				if not self.findkeylf(lhssent):
					if not self.findkeylf(rhssent):
						continue
					else:
						
						if "the same" in " ".join(rhssent):
							# print
							rule = {
							"lhs": "x",
							"predicate": 32,
							...
							rules["keyword"] = self.getkeylf(rhssent)
							rules["type"] = 1
							break
						else:
							continue
	...
	def verb_keywords_match(self, sent, pos, err):
		...
		if re.findall("(.*) (VBZ|VBP) RB ", pos):
			...
			if	re.findall("(.*) (VBZ|VBP) CD", pos):
				...
				# logging.info(lhssent)
				# logging.info(rhssent)
				# logging.info(rhstag)

				if "for" in lhssent and "," in lhssent:
					return False, ""
