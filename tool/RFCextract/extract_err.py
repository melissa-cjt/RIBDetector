...
class RFC_ERR_Rules_Extract(Extract, RFC_Extract):
	...
	def __init_(self, meta_info={}):
		...
		self.meta_errcode = meta_errcode
		self.meta_subcode = meta_subcode
		self.pkt_rul = pkt_rul
	...
	def generate_err_metainfo(self):
		...
		with open("../output/tmp/meta-info-err.json",'w') as f:
	...
	def pre_read_pkt_rule(self):
		...
		if os.path.exists("../output/tmp/rule-pkt.json"):

			f = open("../output/tmp/rule-pkt.json","r")
			# print "ok!"
			context = f.read()
			self.pkt_rul = json.loads(context)
			f.close()
			# print "[LOG] Find the pre-pkt rule!"
		else:
			print "[WARN] There is no pre-pkt rule!"
	...
	def write_in_json(self, err_rules):
		...
		for errcode, subcode in err_rules.items():
			...
			for subc, subcrs in subcode.items():
				...
				rule["error_code"]=[int(self.meta_errcode[errcode]), int(self.meta_subcode[errcode][subc])]
				rule["rfc_conds"]=subcrs
		...
		if not isExists:
			os.makedirs("../output/tmp")
		with open("../output/tmp/rule-err.json",'w') as f:
